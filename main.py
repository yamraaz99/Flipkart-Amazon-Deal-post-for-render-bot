...
# ---------------------------------------------------------------------------
# CONNECTION POOLING (Fixes Render network overhead and speeds up async requests)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. BUYHATKE & HISTORICAL APIs (Uses optimized shared AsyncClientPool)
# ---------------------------------------------------------------------------
async def api_product_details(url):
    try:
        client = AsyncClientPool.get_client()
        r = await client.post(
            f"https://ext1.buyhatke.com/extension-apis/chatBot/fetchProductDetails?extId={EXT_ID}&extAuth={EXT_AUTH}",
            json={"url": url},
...
async def api_product_data(pid, pos):
    try:
        client = AsyncClientPool.get_client()
        r = await client.get(
            f"https://buyhatke.com/api/productData?pos={pos}&pid={pid}",
            timeout=10,
        )
        d = r.json()
        return d.get("data", {}) if d.get("status") == 1 else {}
...
async def shorten_title_groq(full_title):
    if not GROQ_API_KEY:
        return full_title if len(full_title) <= 70 else full_title[:70] + "..."
    try:
        client = AsyncClientPool.get_client()
        resp = await client.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You shorten e-commerce product titles. Keep: brand, key specs (size, capacity, star rating, color), product type. Remove: model codes, marketing buzzwords, AI features, pipe-separated feature lists, processor names. Max ~80 characters. Return ONLY the title, nothing else.",
                    },
...
# ---------------------------------------------------------------------------
# 5. OPTIMIZED WATERMARK ENGINE (Uses 95% less CPU / memory than old code)
# ---------------------------------------------------------------------------
def apply_repeating_watermark(img, text="AmazingDealsLoots"):
    ...
    # Fast paste directly onto base using the stamp itself as the alpha mask
    # Completely avoids slow full-canvas alpha_composite calculations
...
# ---------------------------------------------------------------------------
# 6. HTML TEMPLATES & IMAGE GENERATION
# ---------------------------------------------------------------------------
...
product
Coupon Discount
Save {{ coupon_display_text }} with coupon Apply
{% if coupon_disc > 0 %}
{% endif %}
Items: {{ price_fmt }}.00
Delivery: 0.00
Total: {{ price_fmt }}.00
{% if savings_count > 0 %}
Savings ({{ savings_count }}): −{{ total_savings_fmt }}.00
{% if best_bank_disc > 0 %}
{{ best_bank }} Discount: −{{ best_bank_disc_fmt }}.00
{% endif %} {% if coupon_disc > 0 %}
Your Coupon Savings −{{ coupon_disc_fmt }}.00
{% endif %}
{% endif %}
Order Total: {{ effective_fmt }}.00
""" )

FLIPKART_DEAL_TEMPLATE = Template( """
product
MRP (incl. of all taxes) {{ mrp_fmt }}
{% if has_any_discount %}
Discounts 
{% if show_mrp_discount %}
MRP Discount −{{ mrp_discount_fmt }}
{% endif %} {% if coupon_disc > 0 %}
Coupons for you −{{ coupon_disc_fmt }}
{% endif %} {% if best_bank_disc > 0 %}
Bank Offer Discount −{{ best_bank_disc_fmt }}
{% endif %}
Total Amount {{ effective_fmt }}
{% else %}
Selling Price {{ effective_fmt }}
{% endif %}
""" )
...
async def cmd_optimized(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_mode == 'standard':
        ...
        await update.message.reply_text("Default mode set to **Optimized**.\nAll future links will generate the optimized post first. You can still use the inline button to switch templates.")
    else:
        context.user_data['default_mode'] = 'standard'
        await update.message.reply_text("Default mode set to **Standard**.\nAll future links will generate the standard post first. You can still use the inline button to switch templates.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Phase 1: parallel API calls (using shared connection pool)
        ...
        deal_img = await asyncio.to_thread(
            ...
            marketplace=mkt,
            template_type=default_mode,
            short_title=short_title,
            reg_price=reg_price,
        )

        if default_mode == "optimized":
            btn_text = " Show Standard Version"
            btn_cb = "std_version"
        else:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    deal_img = await asyncio.to_thread(
        ...
        short_title=cache['short_title'],
        reg_price=cache['reg_price'],
    )
    if deal_img:
